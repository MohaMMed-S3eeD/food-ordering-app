...
const BtnCreateAdmin = ({ userId }: { userId: string }) => {
  ...
  return (
    <div className="relative flex justify-center items-center gap-2 pb-4 mx-auto max-w-2xl bg-white shadow-md  ">
      <Button className="flex-1 mx-6 " onClick={handleCreateAdmin}>{t("createAdmin")}</Button>
      <Button className="flex-1 mx-6 " onClick={handleDeleteUser} variant="destructive">
        {t("deleteUser")}
      </Button>
    </div>
  );
